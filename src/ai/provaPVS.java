...
public class provaPVS {
		 ...
		 */
	    public static double zWSearch(State state, int beta, long WP, long WN, long WB, long WR, long WQ, long WK, long BP, long BN, long BB, long BR, long BQ, long BK, long EP, boolean CWK, boolean CWQ, boolean CBK, boolean CBQ, boolean WhiteToMove, int depth) {
	        ...
	        if (depth == 5)
	        {
	            score = HeuristicTablut.getStateValue(state);
	            return score;
	        }
	        ...
	        if (WhiteToMove) {
	            moves = getPossibleMovesW(state);
	        } else {
	            moves = getPossibleMovesB(state);
	        }
